...
p1 <- wisc %>% 
  ggplot(aes(x=V_P, fill = adhd_or_asd)) +
  geom_histogram() +
  ggh4x::facet_grid2(rows = vars(sex), scales = "free_x", space = "free") +
...
#####
# identify users to be contacted
contact.ids <- wisc %>%
  filter(age < 18)  %>%
  mutate(abs_diff = abs(V_P), Record.ID = as.character(Record.ID)) %>%
  arrange(desc(abs_diff))
write_tsv(contact.ids, "data/derivatives/ids-to-contact.tsv")
######
