################################################################################
#                    extract facial landmarks and features in spark            #
################################################################################
rm(list = ls())
gc()
...
setwd(project.dir)
################################################################################
landmarks <- cbind(read_csv("/wdata/atener/face/2023/facial_landmarking/final_photos_coords.csv")) %>%
  mutate(PID = sub(".*S", "S", filename),
         PID = sub("\\.png", "", PID)) %>%
  select(-QCPASS)
...
distances <- foreach(j = 1:nrow(landmarks), .combine = rbind) %dopar% {
  p.landmarks <- landmarks[j,] %>%
    ...
    mutate(coord2 = ifelse(grepl("x", coord), "x", "y"),
           coord3 = sub("x", "", coord),
           coord3 = sub("y", "", coord3)) %>%
    pivot_wider(names_from = "coord2", values_from = "value", id_cols = c("PID", "coord3")) %>%
  ...
  d_m %>%
    mutate(PID = sub("face_", "", landmarks$PID[j]),
           PID = sub("\\.jpg", "", PID))
}
...
ggsave(p, filename = "figs/distribution-of-landmarks-pairs-distances-in-spark.png", bg = "white",
...
wider.distances <- distances %>%
  ...
  mutate(pair = ifelse(is.na(label), pair, label),
         pair = paste0("P_", pair)) %>%
  pivot_wider(names_from = "pair", values_from = "distance", id_cols = "PID")
write_csv(wider.distances, "data/derivatives/pairs-distances-in-spark.csv")
...
a3 <- c(27,35,45,46,47,42,27) # CHK_I_R
...
a9 <- c(27,31,36,41,40,39,27) # CHK_I_L
...
ggsave(p, filename = "figs/distribution-of-landmarks-areas-in-spark.png", bg = "white",
       height = 8, width = 9, units = "in", dpi = 360)
#
################################################################################
wider.facial.areas <- facial.areas %>%
  mutate(area_label = paste0("A_", area_label)) %>%
  pivot_wider(names_from = "area_label", values_from = "area", id_cols = "PID")
write_csv(wider.facial.areas, "data/derivatives/facial.areas-in-spark.csv")
################################################################################
