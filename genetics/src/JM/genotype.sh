...
OUT_PATH=/Dedicated/jmichaelson-wdata/jmichaelson/NDVR/genomics/pilot_96
...
do
	...
	BAM=${OUT_PATH}/${SAMPLE}_recal.bam
	# Check if BAM file exists
	if [ ! -f "$BAM" ]; then
		echo "Error: BAM file $BAM not found for sample $SAMPLE"
		exit 1
	fi
	mkdir -p ${OUT_PATH}/glimpse/${SAMPLE}
	...
	do   
		...
		OUT=${OUT_PATH}/glimpse/${SAMPLE}/${SAMPLE}_imputed
		...
		# If the number of background jobs is >= MAX_JOBS, wait for any job to complete
